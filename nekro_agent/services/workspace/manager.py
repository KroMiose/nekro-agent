...
{extra_section}"""
    ...
    async def add_mcp_server(workspace: DBWorkspace, server: Any) -> None:
        ...
        如果 server.validation.status == 'validated'（前端在保存前已成功跑过握手），
        则携带该状态写入，避免用户重复点验证；否则默认未验证。
        """
        async with _get_mcp_lock(workspace.id):
            ...
            raw["validation"] = WorkspaceService._validation_from_server(server)
    ...
    async def update_mcp_server(workspace: DBWorkspace, old_name: str, server: Any) -> None:
        ...
        async with _get_mcp_lock(workspace.id):
            ...
            # 优先级：前端带过来已验证 > 旧状态同指纹保留 > 重置未验证
            client_validation = WorkspaceService._validation_from_server(server)
            if client_validation.get("status") == "validated":
                new_raw["validation"] = client_validation
            elif old_validation and old_fingerprint == new_fingerprint and server.name == old_name:
    ...
    @staticmethod
    def _validation_from_server(server: Any) -> Dict[str, Any]:
        """从入参的 McpServerConfig.validation 提取已验证状态（若存在），否则未验证

        admin-only 接口，信任前端在 submit 前刚跑完的 /test 结果。前端在配置发生
        修改后会自行清掉 validation，所以收到 status='validated' 时即表示该套配置
        的最近一次握手是成功的。
        """
        validation = getattr(server, "validation", None)
        if validation is None:
            return WorkspaceService._unvalidated_state()
