...
async def cc_workspace_status(_ctx: schemas.AgentCtx) -> str:
    ...
    result = (
        ...
        f"\n"
        f"[KB Boundary]\n"
        f"- Use CC Workspace for repository source code inspection, implementation details, debugging, refactors, command execution, file editing, PR work, and any task that depends on the live codebase.\n"
        f"- Use Knowledge Base tools only for static documents already indexed into the workspace KB, such as imported manuals, FAQ, rules, and reference docs.\n"
        f"- If both could help, inspect the live codebase with CC first, and use KB only as supplementary background material.\n"
        f"- Do not delegate a pure KB lookup to CC when the answer should come from indexed static documents.\n"
        f"\n"
