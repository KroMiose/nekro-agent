...
CONTAINER_SHARE_DIR = "/app/shared"  # 容器内共享目录
...
async def run_code_in_sandbox(code_text: str, output_limit: int = 1000) -> str:
    ...
    Path.write_text(code_file_path, code_text)
    ...
    container: DockerContainer = await docker.containers.run(
        ...
        config={
            ...
            "HostConfig": {
                "Binds": [f"{HOST_DIR}:{CONTAINER_SHARE_DIR}"],
    ...
    # 等待容器执行并限制时间
    output_text = await run_container_with_timeout(container, config.SANDBOX_RUNNING_TIMEOUT)
    return (
        output_text
